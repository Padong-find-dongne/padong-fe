...
interface BoundaryStore {
  boundaries: Record<string, any>; // key: adminDongCode, value: boundary data
  fetchBoundary: (adminDongCode: string) => Promise<any | null>;
  clearBoundaries: () => void;
}

export const useBoundaryStore = create<BoundaryStore>((set, get) => ({
  boundaries: {},

  fetchBoundary: async (adminDongCode: string) => {
